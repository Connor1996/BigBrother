...
我们平时处理 PR，真正花时间的往往不是“改代码”本身，而是围着代码变化不断确认上下文。
...
还有一个边界也很重要：BigBrother 不是在定义“实现阶段应该怎么做”。

在真正开始做需求、讨论方案、权衡设计、反复修改实现的时候，我们通常还是需要和 agent 做多轮来回的交互。除了偏 web coding 这类比较容易一次性推进的场景，大多数实现工作都还是要靠这种反复对话去打磨。现在大家在这一阶段用的方法本来就很多，可以是 Slack AI，可以是 agent team，可以是 Codex 加 cloud subagents，也可以是各自更顺手的别的组合。这一段工作流今天仍然是开放的，不太适合被单一产品形态硬性收口。

但 PR 一旦提出来，后面的长尾问题就不是另一种“开放式创作”，而是一条相对确定的运营流程了。你需要不断回来看 comment、CI、mergeability、review request、failed run、needs decision。也正因为这一段流程比较确定，它才更适合被一个持续可见的 dashboard 和一套明确的操作控件承接下来。这就是 BigBrother 想解决的那一段。
...
When we work through pull requests, the expensive part is often not the code change itself. It is the repeated checking that surrounds the code change.
...
There is another boundary that matters just as much: BigBrother is not trying to define how implementation work should happen.

When people are still shaping a requirement, discussing tradeoffs, exploring a design, and iterating on the implementation itself, they usually do need repeated back-and-forth with an agent. Outside of a few workflows, such as some web-coding tasks that can be driven more linearly, most real implementation work still benefits from that multi-turn refinement loop. And at that stage, teams already use many different setups: Slack AI, agent teams, Codex with cloud subagents, or whatever combination fits their habits best. That part of the workflow is still open-ended, and it is probably not something one product should try to over-standardize.

Once a PR is open, though, the problem changes. The long tail after the PR is raised is much less like open-ended creation and much more like a repeatable operational loop. You have to keep checking comments, CI, mergeability, review requests, failed runs, and `needs decision` states. Because that workflow is much more structured, it is exactly the part that benefits from a persistent dashboard and a small set of explicit controls. That is the slice BigBrother is designed to own.
