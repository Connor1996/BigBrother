...
- the daemon should load those templates from the repository's `prompts/*.md` files so operators
...
- `POST /api/review-requests/deep-review` with `{ "key": "<repo>#<number>" }`
  ...
  uses the repo-vendored `$deep-review` skill to write a final markdown review artifact, persists that final report,
...
- the embedded terminal should reserve a separate inner content gutter and outer scrollbar gutter on the right, so the text keeps breathing room while the scrollbar handle sits beside the content area instead of on top of the rightmost columns
