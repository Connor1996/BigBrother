...
```text
You are setting up BigBrother for local use in the `symphony-rs` repository on this machine.
...
Then create or update `symphony-rs.toml`. If the file does not exist, copy `symphony-rs.example.toml`. If it already exists, preserve the operator's intent and patch it carefully instead of replacing it wholesale. Set up the config so it matches the current machine.
