...
Start by inspecting the machine. Confirm whether `gh` is installed and authenticated, whether `codex` is available, whether Git can push to the operator's normal remotes, and which likely source repositories exist under the operator's shared code directory such as `~/Coding`. If `GITHUB_TOKEN` or `GH_TOKEN` is already set, note that; otherwise, if `gh` is authenticated, use `gh auth token` as the default way to bootstrap local setup. If you need the GitHub login, prefer discovering it from GitHub itself with `gh api user -q .login` instead of guessing.
...
For the GitHub section, prefer `api_token = "$GITHUB_TOKEN"`. If the operator does not already export `GITHUB_TOKEN` or `GH_TOKEN`, but `gh` is authenticated, bootstrap the current shell with `export GITHUB_TOKEN="$(gh auth token)"` before launching BigBrother and tell the operator that this environment variable still needs to be present in future sessions. If `author = "$GITHUB_USER"` would remain unresolved, either replace it with the real GitHub login you discovered, export `GITHUB_USER="$(gh api user -q .login)"`, or remove the `author` field so BigBrother can resolve the viewer login dynamically at runtime. Do not leave a literal unresolved placeholder there.
